<script lang="ts">
    ...
    $effect(() => {
        if (clientStateObject.state.gameState !== "select-background-fail") {
...
<p>
    The suspect will now recieve a role. Take this time to review the {#if multiplePenalties}penalties{:else}penalty{/if} and module prompts and prepare to interrogate the suspect.
</p>
