<script lang="ts">
    ...
    afterNavigate(() => availableBackgrounds = knuthShuffle(backgroundData.slice(1)).slice(0, 1));
...
<Button disabled={disableSelectBackground} type="submit" onclick={async () => await submitBackground()} class="w-fit mx-auto h-auto py-3"><h3 class="text-wrap">Ready for interrogation</h3></Button>
